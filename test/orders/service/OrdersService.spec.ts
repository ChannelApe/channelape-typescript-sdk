...
import { JsonOrderFormatterService, OrderStatus } from '../../../src';
...
Code: 15 Message: Requested business cannot be found.`;
    ...
    it(`And valid order when updating said order with no actionId
      ...
      mockedAxiosAdapter.onPut(`${Environment.STAGING}/${Version.V1}${Resource.ORDERS}/${order.id}`)
        .reply((data) => {
          ...
          expect(data.headers['X-Channel-Ape-Authorization-Token']).to.equal('valid-session-id');
          const requestOrder = JsonOrderFormatterService.formatOrder(data.data);
          expect(requestOrder.lineItems[0].taxes!.length).to.equal(2);
          expect(requestOrder.lineItems[0].taxes![0].title).to.equal('PA State Tax');
          expect(requestOrder.lineItems[0].taxes![0].price).to.equal(9.99);
          expect(requestOrder.lineItems[0].taxes![0].rate).to.equal(0.06);
          expect(requestOrder.lineItems[0].taxes![1].title).to.equal('NV State Tax');
          expect(requestOrder.lineItems[0].taxes![1].price).to.equal(1.00);
          expect(requestOrder.lineItems[0].taxes![1].rate).to.be.undefined;
          expect(requestOrder.lineItems[1].taxes!.length).to.equal(0);
          return Promise.resolve([202, singleOrderToUpdateResponse]);
        });
      ...
      return ordersService.update(order).then((actualOrder) => {
        ...
        expect(actualOrder.fulfillments![0].lineItems.length).to.equal(2);
        expect(actualOrder.fulfillments![0].lineItems[0].sku).to.equal('b4809155-1c5d-4b3b-affc-491ad5503007');
        expect(actualOrder.lineItems[0].taxes!.length).to.equal(2);
        expect(actualOrder.lineItems[0].taxes![0].title).to.equal('PA State Tax');
        expect(actualOrder.lineItems[0].taxes![0].price).to.equal(9.99);
        expect(actualOrder.lineItems[0].taxes![0].rate).to.equal(0.06);
        expect(actualOrder.lineItems[0].taxes![1].title).to.equal('NV State Tax');
        expect(actualOrder.lineItems[0].taxes![1].price).to.equal(1.00);
        expect(actualOrder.lineItems[0].taxes![1].rate).to.be.undefined;
        expect(actualOrder.lineItems[1].taxes!.length).to.equal(0);
      });
    });
    ...
    it('And valid OrderCreateRequest with an actionId when creating an order, Then return created order', () => {
      ...
      return ordersService.create(orderCreateRequest).then((createdOrder) => {
        ...
        expect(createdOrder.lineItems[0].taxes![0].rate).to.equal(0.06);
        expect(createdOrder.lineItems[0].taxes![1].title).to.equal('NV State Tax');
        expect(createdOrder.lineItems[0].taxes![1].price).to.equal(1.00);
        expect(createdOrder.lineItems[0].taxes![1].rate).to.be.undefined;
      });
    });
