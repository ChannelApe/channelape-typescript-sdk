...
describe('Session Retrieval Service', () => {
  ...
  describe('given some rest client and some given endpoint', () => {
    ...
    afterEach((done) => {
      (someClient.post as any).restore();
      done();
    });
    ...
    it('given valid session request' +
      ...
      const someClientPostStub = sinon.stub(someClient, 'post').callsFake(mockResponse(expectedResponse, 200));
      ...
      return sessionRetrievalService.retrieveSession({
        ...
        expect(someClientPostStub.args[0][0]).to.equal(`${someEndpoint}/v1/sessions`);
    ...
    it('given valid session request ' +
      ...
      const someClientPostStub = sinon.stub(someClient, 'post').callsFake(() => emitter);
