...
describe('ChannelApe Client', () => {
  describe('Given valid session ID', () => {
    ...
    describe('And valid inventory batch request', () => {
      context('When creating inventory batch', () => {
        it('Then create batch and return response', () => {
          ...
          return actualBatchPromise.then((actualCreatedBatch) => {
            expect(actualCreatedBatch.businessId).to.equal(
              '4baafa5b-4fbf-404e-9766-8a02ad45c3a4'
            );
          });
        });
      });
    ...
    describe('And valid action ID for action with error processing status', () => {
      context('When retrieving action', () => {
        it('Then return action', () => {
          ...
          const actualActionPromise = channelApeClient
            .actions()
            .get(expectedActionId);
          return actualActionPromise.then((actualAction) => {
            expect(actualAction.action).to.equal('PRODUCT_PULL');
            expect(actualAction.businessId).to.equal(
              '4baafa5b-4fbf-404e-9766-8a02ad45c3a4'
            );
            expect(actualAction.description).to.equal(
              'Encountered error during product pull for Europa Sports'
            );
            expect(actualAction.healthCheckIntervalInSeconds).to.equal(300);
            expect(actualAction.id).to.equal(expectedActionId);
            expect(actualAction.lastHealthCheckTime.toISOString()).to.equal(
              new Date('2018-04-24T14:02:34.703Z').toISOString()
            );
            expect(actualAction.processingStatus).to.equal('error');
            expect(actualAction.startTime.toISOString()).to.equal(
              new Date('2018-04-24T14:02:34.703Z').toISOString()
            );
            expect(actualAction.targetId).to.equal(
              '1e4ebaa6-9796-4ccf-bd73-8765893a66bd'
            );
      ...
      context('When retrieving action', () => {
        it('Then return action', () => {
          ...
          return actualActionPromise.then((actualAction) => {
            ...
            expect(actualAction.description).to.equal(
              'Completed product push for Custom Column Export'
            );
            expect(actualAction.healthCheckIntervalInSeconds).to.equal(300);
            expect(actualAction.id).to.equal(expectedActionId);
            expect(actualAction.lastHealthCheckTime.toISOString()).to.equal(
              new Date('2018-05-01T14:47:58.018Z').toISOString()
            );
            expect(actualAction.processingStatus).to.equal('completed');
            expect(actualAction.startTime.toISOString()).to.equal(
              new Date('2018-05-01T14:47:55.905Z').toISOString()
            );
            expect(actualAction.targetId).to.equal(
              '9c728601-0286-457d-b0d6-ec19292d4485'
            );
            ...
            if (actualAction.endTime == null) {
              ...
              expect(actualAction.endTime.toISOString()).to.equal(
                new Date('2018-05-01T14:47:58.018Z').toISOString()
              );
            }
          });
        ...
        it('Then return 404 status code and action not found error message', () => {
          ...
          return actualActionPromise
            ...
            .catch((actualChannelApeError: ChannelApeError) => {
              ...
              assertChannelApeErrors(
                expectedChannelApeApiErrors,
                actualChannelApeError.ApiErrors
              );
            });
        });
      });
    ...
    describe('And valid action ID for action with completed processing status', () => {
      context('When updating action', () => {
        it('Then expect action already completed', () => {
          const expectedActionId = '4da63571-a4c5-4774-ae20-4fee24ab98e5';
          return channelApeClient
            .actions()
            .updateHealthCheck(expectedActionId)
            .then((actualAction) => {
    ...
    describe('And valid channel ID', () => {
      context('When retrieving channel', () => {
        it('Then return channel', () => {
          ...
          const actualChannelPromise = channelApeClient
            .channels()
            .get(expectedChannelId);
    ...
    describe('And valid supplier ID', () => {
      context('When retrieving supplier', () => {
        it('Then return supplier', () => {
          ...
          const actualSupplierPromise = channelApeClient
            .suppliers()
            .get(expectedSupplierId);
          return actualSupplierPromise.then(
            assertSupplierEuropaSportsSnackFoods
          );
        });
      });
    });
    ...
    describe('And valid supplier', () => {
      context('When updating a supplier', () => {
        it('Then return the supplier', () => {
          ...
          const actualSuppliersPromise = channelApeClient
            .suppliers()
      ...
      context('When retrieving order', () => {
        it('Then return order', () => {
          ...
          return actualOrderPromise.then((actualOrder) => {
            ...
            expect(actualOrder.lineItems[0].title).to.equal(
              'Awesome Cotton Ball'
            );
            expect(actualOrder.fulfillments!.length).to.equal(1);
            const expectedShippedAt = new Date('2019-02-11T22:39:11.407Z');
            expect(
              actualOrder.fulfillments![0].shippedAt!.toISOString()
            ).to.equal(expectedShippedAt.toISOString());
            const expectedCreatedAt = new Date('2019-02-11T22:39:13.397Z');
            expect(actualOrder.createdAt.toISOString()).to.equal(
      ...
      context('When retrieving order with a refund', () => {
        it('Then return order', () => {
          ...
          return actualOrderPromise.then((actualOrder) => {
            ...
            expect(actualOrder.refunds![0].channelRefundId).to.equal(
              '74273487234'
            );
            expect(actualOrder.refunds![0].supplierRefundId).to.equal(
              '7348234'
            );
          });
        });
          ...
          return channelApeClient
            ...
            .then((actualOrder) => {
              ...
              return channelApeClient
                .orders()
                .update(actualOrder)
                .then((actualUpdatedOrder) => {
                  expect(actualUpdatedOrder.id).to.equal(actualUpdatedOrder.id);
                  expect(actualUpdatedOrder.customer!.firstName!).to.equal(
                    randomFirstName
                  );
                  expect(actualUpdatedOrder.customer!.lastName!).to.equal(
                    randomLastName
                  );
        ...
        it('Then patch the order', () => {
          ...
          return channelApeClient
            ...
            .then((actualOrder) => {
              ...
              actualOrder.customer!.shippingAddress!.provinceCode =
                fakeProvinceCode;
              return channelApeClient
                .orders()
                .patch(actualOrder)
                .then((actualUpdatedOrder) => {
                  expect(actualUpdatedOrder.id).to.equal(actualUpdatedOrder.id);
                  expect(
                    actualUpdatedOrder.customer!.shippingAddress!.address1
                  ).to.equal(fakeAddress);
                  expect(
                    actualUpdatedOrder.customer!.shippingAddress!.city
                  ).to.equal(fakeCity);
                  expect(
                    actualUpdatedOrder.customer!.shippingAddress!.province
                  ).to.equal(fakeProvince);
                  expect(
                    actualUpdatedOrder.customer!.shippingAddress!.provinceCode
                  ).to.equal(fakeProvinceCode);
                });
            });
        });
      });
      ...
      context('When creating an order', () => {
        it('Then create the order', () => {
          ...
          return channelApeClient
            ...
            .then((createdOrder) => {
              expect(createdOrder.purchaseOrderNumber).to.equal(
                expectedPurchaseOrderNumber
              );
              expect(createdOrder.businessId).to.equal(expectedBusinessId);
              expect(createdOrder.totalPrice).to.equal(
                orderToCreate.totalPrice
              );
              expect(createdOrder.additionalFields![0].name).to.equal('name');
              expect(createdOrder.additionalFields![0].value).to.equal(
                orderToCreate.additionalFields![0].value
              );
              expect(createdOrder.additionalFields![1].name).to.equal(
                'order_number'
              );
              expect(createdOrder.additionalFields![1].value).to.equal(
                orderToCreate.additionalFields![1].value
              );
              expect(createdOrder.channelId).to.equal(
                expectedChannelId,
                'channelId'
              );
              expect(createdOrder.customer!.firstName!).to.equal(
                expectedFirstName,
                'customer.firstName'
              );
              expect(createdOrder.customer!.lastName!).to.equal(
                expectedLastName,
                'customer.lastName'
              );
              expect(createdOrder.customer!.additionalFields![0].name).to.equal(
                orderToCreate.customer!.additionalFields![0].name
              );
              expect(
                createdOrder.customer!.additionalFields![0].value
              ).to.equal(orderToCreate.customer!.additionalFields![0].value);
              expect(createdOrder.customer!.name!).to.equal(
                fullName,
                'customer.name'
              );
              expect(createdOrder.lineItems.length).to.equal(
                4,
                'line item length'
              );
              expect(createdOrder.channelOrderId).to.equal(
                expectedChannelOrderId
              );
              for (let i = 0; i < 4; i += 1) {
                expect(createdOrder.lineItems[i].id).to.equal(
                  (i + 1).toString(),
                  `lineItem[${i}].id`
                );
                expect(createdOrder.lineItems[i].quantity).to.equal(
                  expectedLineItemQuantities[i],
                  `lineItem[${i}].quantity`
                );
                expect(createdOrder.lineItems[i].title).to.equal(
                  expectedLineItemTitles[i],
                  `lineItem[${i}].title`
                );
                expect(
                  createdOrder.lineItems[i].additionalFields![0].name
                ).to.equal(
                  orderToCreate.lineItems[i].additionalFields![0].name
                );
                expect(
                  createdOrder.lineItems[i].additionalFields![0].value
                ).to.equal(
                  orderToCreate.lineItems[i].additionalFields![0].value
                );
              }
            });
        });
          ...
          return channelApeClient
            ...
            .create({
              ...
              messages: [
                {
                  description:
                    'Order was updated by ChannelApe SDK e2e test suite using a channelId and channelOrderId',
          ...
          return channelApeClient
            ...
            .create({
              ...
              messages: [
                {
                  description:
                    'Order was updated by ChannelApe SDK e2e test suite using just an orderId.',
                  title: 'CA SDK e2e test',
                },
              ],
            })
          ...
          return channelApeClient
            ...
            .create({
              ...
              messages: [
                {
                  description:
                    'Order was updated by ChannelApe SDK e2e test suite using a channelOrderId and businessId',
                  title: 'CA SDK e2e test',
                },
              ],
            })
    ...
    describe('And valid business ID', () => {
      describe('And a startDate of "2018-03-29T17:00:51.000Z" and an endDate of "2018-08-23T12:41:33.000Z"', () => {
        context('When retrieving orders', () => {
          it('Then return the 230 orders between those dates', () => {
            ...
            const actualOrdersPromise = channelApeClient
              .orders()
              .get(ordersQueryRequestByBusinessId);
            return actualOrdersPromise.then((actualOrders) => {
              expect(actualOrders).to.be.an('array');
              expect(actualOrders.length).to.equal(230);
              expect(actualOrders[0].id).to.equal(
                'dda8a05f-d5dd-4535-9261-b55c501085ef'
              );
            });
          });
        });
      ...
      describe('And valid business ID', () => {
        describe('And an updatedStartDate of "2018-03-29T17:00:51.000Z" and an updatedEndDate of "2018-08-23T12:41:33.000Z"', () => {
          context('When retrieving orders', () => {
            it('Then return the 226 orders between those dates', () => {
              const expectedBusinessId = '4baafa5b-4fbf-404e-9766-8a02ad45c3a4';
              const ordersQueryRequestByBusinessId: OrdersQueryRequestByBusinessId =
                {
                  businessId: expectedBusinessId,
              ...
              return actualOrdersPromise.then((actualOrders) => {
                expect(actualOrders).to.be.an('array');
                expect(actualOrders.length).to.equal(226);
                expect(actualOrders[0].id).to.equal(
                  '972ab091-ccd9-4400-9154-c41fe9a802ec'
                );
              });
            }).timeout(180000);
          });
          ...
          context('When retrieving orders', () => {
            it('Then expect 1 matching order to be returned', () => {
              ...
              return actualOrdersPromise.then((actualOrders) => {
                ...
                expect(actualOrders[0].id).to.equal(
                  '36d00c15-a2a8-43d3-8c4c-f62bba7ade53'
                );
                expect(actualOrders[0].purchaseOrderNumber).to.equal(
                  ordersQueryRequestByBusinessId.purchaseOrderNumber
                );
              });
            });
          });
        ...
        describe('And query request size of 150 And business has more than 150 orders', () => {
          context('When retrieving a single page of orders', () => {
            it('Then return a single page of 150 orders for the business', () => {
              ...
              const actualOrdersPromise = channelApeClient
                .orders()
                .getPage(ordersQueryRequestByBusinessId);
              return actualOrdersPromise.then((actualOrders) => {
            ...
            context('When retrieving the next single page of orders', () => {
              it('Then return the last single page of 74 orders for the business', () => {
                ...
                return actualOrdersPromise.then((actualOrders) => {
                  ...
                  expect(actualOrders.orders[0].id).to.equal(
                    'a6f23ae7-fae6-4cf3-b7b9-10eaa84d7ff2'
                  );
          ...
          context('When retrieving a single page of orders', () => {
            it('Then return a single page of 10 orders for the business and a totalItems count of "140"', () => {
              ...
              return actualOrdersPromise.then((actualOrders) => {
                ...
                expect(actualOrders.pagination.lastPage).to.equal(false);
                expect(actualOrders.pagination.totalItems).to.equal(
                  140,
                  'There should be 140 totalItems in the pagination response'
                );
              });
            });
          });
        ...
        describe('and locations exist', () => {
          context('When retrieving locations', () => {
            it('Then return the locations', async () => {
              const expectedBusinessId = '4baafa5b-4fbf-404e-9766-8a02ad45c3a4';
              const actualLocations = await channelApeClient
                .locations()
                .getByBusinessId(expectedBusinessId);
      ...
      describe('And valid productId', () => {
        context('When retrieving a products variants', () => {
          it('Then return variants for that product', () => {
            ...
            const actualVariantsPromise = channelApeClient
              .variants()
              .get(variantsRequestByProductId);
      ...
      describe('And valid productId and valid variantId', () => {
        context('When retrieving a variant', () => {
          it('Then return that variant', () => {
            ...
            const actualVariantPromise = channelApeClient
              .variants()
              .get(variantsRequest);
            return actualVariantPromise.then((actualVariant) => {
              expect(actualVariant.options.Flavor).to.equal('Chocolate');
              expect(actualVariant.additionalFields.walmartBrand).to.equal(
                'MusclePharm'
              );
              expect(actualVariant.retailPrice).to.equal(59.99);
              expect(actualVariant.title).to.equal(
                'MusclePharm Sport Series Combat XL Mass Gainer'
              );
            });
          });
        });
      ...
      describe('And valid productFilterId', () => {
        context('When searching variants', () => {
          it('Then return variant quick search results', () => {
            const expectedBusinessId = '4baafa5b-4fbf-404e-9766-8a02ad45c3a4';
            const expectedProductFilterId =
              'f4cf2afd-fc5f-424d-bf45-868b672d77a0';
        ...
        and a size is specified`,
          () => {
            it('Then return variant quick search results according to size', () => {
              ...
              const actualVariantsPromise = channelApeClient
                .variants()
                .getPage(variantsRequest);
              return actualVariantsPromise.then(
                (variantsPage: VariantsPage) => {
                  expect(variantsPage.variantSearchResults).to.be.an('array');
                  expect(variantsPage.variantSearchResults.length).to.be.equal(
                    10
                  );
                }
              );
      ...
      describe('And valid businessId and valid sku', () => {
        ...
        context('When searching inventory items', () => {
          it('Then return inventory item results', async () => {
            ...
            const actualInventoryItems = await channelApeClient
              .inventories()
              .get(expectedBusinessId, expectedSku);
            expect(actualInventoryItems.length).to.equal(1);
            expect(actualInventoryItems[0].businessId).to.equal(
              expectedBusinessId
            );
            expect(actualInventoryItems[0].id).to.equal('30');
            expect(actualInventoryItems[0].sku).to.equal(expectedSku);
            expect(actualInventoryItems[0].title).to.equal(
              "Emmett Brown's Time Traveling Shoes - Mcfly 88's"
            );
            expect(actualInventoryItems[0].createdAt.toString()).to.equal(
              new Date('2019-11-20T13:46:20.000Z').toString()
            );
          ...
          it('Then return variant quick search results', () => {
            ...
            return actualVariantsPromise.then((actualVariants) => {
              ...
              const variant = actualVariants.find(
                v => v.productId === expectedProductId
              );
              expect(variant!.businessId).to.equal(expectedBusinessId);
              expect(variant!.vendor).to.equal('Caveman Foods');
      ...
      describe('And valid businessId with an active subscription', () => {
        context('When getting subscription', () => {
          it('Then return subscription result', () => {
            const expectedBusinessId = '4baafa5b-4fbf-404e-9766-8a02ad45c3a4';
            const actualSubscriptionsPromise = channelApeClient
              .subscriptions()
              .get(expectedBusinessId);
            return actualSubscriptionsPromise.then((actualSubscription) => {
              expect(actualSubscription.active).to.equal(true);
              expect(actualSubscription.businessId).to.equal(
                expectedBusinessId
              );
              expect(actualSubscription.createdAt!.toISOString()).to.be.a(
                'string'
              );
              expect(actualSubscription.errors).to.be.an('array');
              expect(actualSubscription.errors.length).to.equal(0);
              expect(
                actualSubscription.lastCompletedTaskUsageRecordingTime!.toISOString()
              ).to.be.a('string');
              expect(actualSubscription.periodEndsAt!.toISOString()).to.be.a(
                'string'
              );
              expect(actualSubscription.periodStartedAt!.toISOString()).to.be.a(
                'string'
              );
              expect(actualSubscription.subscriptionId).to.not.equal(undefined);
              expect(actualSubscription.subscriptionProductHandle).to.not.equal(
                undefined
              );
              expect(actualSubscription.updatedAt!.toISOString()).to.be.a(
                'string'
              );
            });
          });
      ...
      describe('And valid business member get request', () => {
        context('When getting business member', () => {
          it('Then return business member', async () => {
            ...
            const member = await channelApeClient
              .businesses()
              .getBusinessMember(request);
      ...
      describe('And valid inventory item id', () => {
        context('When getting inventory item', () => {
          it('Then return inventory item', async () => {
            ...
            const inventoryItem = await channelApeClient
              .inventories()
              .get('30');
            expect(inventoryItem.businessId).to.equal(businessId);
            expect(inventoryItem.createdAt).to.equal(
              '2019-11-20T13:46:20.000Z'
            );
            ...
            expect(inventoryItem.title).to.equal(
              "Emmett Brown's Time Traveling Shoes - Mcfly 88's"
            );
          });
        });
      ...
      describe('And valid inventory item creation request', () => {
        context('When creating inventory item', () => {
          it('Then create and return inventory item', async () => {
            ...
            const inventoryItem = await channelApeClient
              .inventories()
              .create(inventoryItemCreationRequest);
            expect(inventoryItem.businessId).to.equal(businessId);
      ...
      describe('And valid location creation request', () => {
        context('When creating location', () => {
          it('Then create and return location', async () => {
            const businessId = '4baafa5b-4fbf-404e-9766-8a02ad45c3a4';
            const generatedName = `Some-Location-${new Date()
              .getTime()
              .toString()}`;
      ...
      describe('And valid location update request', () => {
        context('When update location', () => {
          it('Then update and return location', async () => {
            const businessId = '4baafa5b-4fbf-404e-9766-8a02ad45c3a4';
            const generatedName = `Some-Location-${Math.floor(
              Math.random() * 100000 + 1
            ).toString()}`;
      ...
      describe('And valid adjustment request', () => {
        context('When adjusting quantities', () => {
          it('Then update and return quantities', async () => {
            ...
            const actualAdjustment = await channelApeClient
              .inventories()
              .quantities()
            ...
            expect(actualAdjustment.inventoryStatus).to.equal(
              adjustmentRequest.inventoryStatus
            );
          });
        });
      ...
      describe('And valid batch adjustment request', () => {
        context('When adjusting quantities', () => {
          it('Then update', async () => {
            ...
            await channelApeClient
              .inventories()
              .quantities()
              .setBatch(adjustmentsBySku);
          });
        });
      ...
      describe('And valid inventory item id', () => {
        context('When retrieving inventory quantities', () => {
          it('Then return inventory quantities', async () => {
            const quantities = await channelApeClient
              .inventories()
              .quantities()
              .retrieve('35');
      ...
      describe('And valid step ID', () => {
        context('When retrieving step', () => {
          it('Then return step', () => {
            ...
            const actualStepPromise = channelApeClient
              .steps()
              .get(expectedStepId);
    ...
    function assertChannelApeErrors(
      expectedChannelApeErrors: ChannelApeApiError[],
      actualChannelApeErrors: ChannelApeApiError[]
    ) {
      if (Array.isArray(actualChannelApeErrors)) {
        expect(expectedChannelApeErrors.length).to.equal(
          actualChannelApeErrors.length,
          'expected and actual ChannelApeError arrays are different sizes, expected: ' +
            `${JSON.stringify(
              expectedChannelApeErrors
            )}, actual: ${JSON.stringify(actualChannelApeErrors)}`
        );

        expectedChannelApeErrors.sort(
          (leftChannelApeError, rightChannelApeError) =>
            leftChannelApeError.code - rightChannelApeError.code
        );
        actualChannelApeErrors.sort(
          (leftChannelApeError, rightChannelApeError) =>
            leftChannelApeError.code - rightChannelApeError.code
        );
        ...
        expectedChannelApeErrors.forEach((expectedChannelApeError, index) => {
          ...
          expect(actualChannelApeError.code).to.equal(
            expectedChannelApeError.code,
            `Unexpected code for ChannelApeError at index ${index}`
          );
          expect(actualChannelApeError.message).to.equal(
            expectedChannelApeError.message,
            `Unexpected message for ChannelApeError at index ${index}`
          );
        });
      } else if (expectedChannelApeErrors.length > 0) {
        expect(Array.isArray(actualChannelApeErrors)).to.equal(
          true,
          'actualChannelApeErrors should be array'
        );
      }
    }
    ...
    function assertChannelEuropaSportsSnackFoods(channel: Channel) {
      ...
      expect(channel.businessId).to.equal(
        '4baafa5b-4fbf-404e-9766-8a02ad45c3a4'
      );
      expect(channel.enabled).to.equal(true);
      expect(channel.integrationId).to.equal(
        '02df0b31-a071-4791-b9c2-aa01e4fb0ce6'
      );
      ...
      expect(channel.createdAt.toISOString()).to.equal(
        expectedCreatedAt.toISOString()
      );
      expect(channel.updatedAt.getUTCMilliseconds()).to.be.greaterThan(
        expectedCreatedAt.getUTCMilliseconds()
      );
    }

    function assertSupplierEuropaSportsSnackFoods(supplier: Supplier) {
      expect(supplier.businessId).to.equal(
        '4baafa5b-4fbf-404e-9766-8a02ad45c3a4'
      );
      expect(supplier.enabled).to.equal(true);
      expect(
        supplier.fileSettings!.additionalFieldsMapping[0].columnIndex
      ).to.equal(55);
      expect(
        supplier.fileSettings!.additionalFieldsMapping[1].sourceId
      ).to.equal('products');
      expect(supplier.fileSettings!.descriptionMapping[0].columnIndex).to.equal(
        7
      );
      expect(supplier.fileSettings!.descriptionMapping[1].prefix).to.equal(
        '<div class="directions">Directions</div><p>'
      );
      ...
      expect(
        supplier.fileSettings!.primaryCategoryMapping.columnIndex
      ).to.equal(16);
      expect(supplier.fileSettings!.productMapping.columnIndex).to.equal(2);
      expect(supplier.fileSettings!.retailPriceMapping.columnIndex).to.equal(
        19
      );
      expect(supplier.fileSettings!.retailPriceMapping.currencyCode).to.equal(
        'USD'
      );
      ...
      expect(
        supplier.fileSettings!.sources[0].authorization!.passwordKey
      ).to.equal('password');
      expect(supplier.fileSettings!.sources[1].id).to.equal('nutrients');
      expect(supplier.fileSettings!.weightMapping.unitOfMeasurement).to.equal(
        'g'
      );
      expect(supplier.id).to.equal('1e4ebaa6-9796-4ccf-bd73-8765893a66bd');
      expect(supplier.integrationId).to.equal(
        '2eacfed0-46ce-46b5-b8c6-dd8e29672c8c'
      );
      expect(supplier.name).to.equal('Europa Sports');
    }

    function assertStep(actualStep: Step) {
      expect(actualStep.id).to.deep.equal(
        'e3d1046f-a878-4d42-b675-0c215a406075'
      );
      expect(actualStep.name).to.deep.equal('E2E Test Step');
    }

    function assertSupplierUpdate(
      actualSupplier: Supplier,
      updateRequest: SupplierUpdateRequest
    ) {
      ...
      expect(actualSupplier.integrationId).to.equal(
        updateRequest.integrationId
      );
      expect(actualSupplier.stepSettings).to.deep.equal(
        updateRequest.stepSettings
      );
    }
  });
});
